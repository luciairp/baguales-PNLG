...
Agrego: unifico todo lo de brazo SUR y Brazo Rico, y edades a 3 clases
```{r}
datos_ind_sex_gral <- datos_ind %>%
  mutate(
  edad = case_when(
      edad == "1" ~ "ternero",
      edad %in% c("2","3") ~ "joven",
      edad %in% c("4","5") ~ "adulto")) %>% 
  mutate(edad = ordered(fct_relevel(edad, "ternero","joven","adulto"))) %>% 
  group_by(y,sexo,edad, zona) %>% 
  filter(zona == "BR"| zona == "BSO"|zona=="BSE"|zona=="BSS") %>% 
  filter(edad != "NA") %>% 
  filter(y < 2020) %>% 
  summarise(Total = n()) %>% 
  mutate(Pob = ifelse(sexo == "H", -Total,Total))

ggplot(datos_ind_sex_gral,aes(x = edad,y = Pob, fill = sexo)) +
    geom_col(position = "stack", alpha = 0.6) + 
    coord_flip() +
    scale_fill_manual(values = c("red", "blue")) +
    theme_minimal() +
